...
void NonCombatStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("timer", NextAction::array(0, new NextAction("check mount state", 1.0f),
        ...
        nullptr)));
    // Ensure Wintergrasp auto-join even if invite packets are missed
    triggers.push_back(new TriggerNode("timer", NextAction::array(0, new NextAction("wg ensure queued", 0.5f), nullptr)));
